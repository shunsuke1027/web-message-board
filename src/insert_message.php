...
if($is_valid_auther_name && mb_strlen($input_author_name)>30){
    $is_valid_auther_name = false;
    $_SESSION['input_error_author_name']='ニックネームは30字以内で入力してください。(現在'.mb_strlen($input_author_name).'文字)';
}
...
if($is_valid_message && mb_strlen($input_message)>1000){
    $is_valid_message = false;
    $_SESSION['input_error_message']='投稿内容は1000字以下で入力してください。(現在'.mb_strlen($input_message).'文字)';
}
...
if($is_valid_auther_name && $is_valid_message){
    ...
    //insertクエリを作成する
    //:author_name,:messageはプレースホルダという。後で$stmt->bindValueを使用して値をセットするときのニックネームのようなもの。自分で決められる。
    $query='INSERT INTO posts(author_name,message)VALUE(:author_name,:message)';
